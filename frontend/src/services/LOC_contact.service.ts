...
export class ContactService {
  ...
  public getNeighborContactId(contactId, diff){
    let contactIdx = this._contactsDb.findIndex(({_id}) => _id===contactId)
    contactIdx+=diff;
    if(contactIdx<0) contactIdx = this._contactsDb.length-1;
    else if(contactIdx >=this._contactsDb.length) contactIdx=0;
    return this._contactsDb[contactIdx]._id
  }
  ...
  private _addContact(contact: Contact) {
    ...
    this._contactsDb=[...this._contactsDb, newContact]
    this._contacts$.next(this._sort(this._contactsDb))
  }
